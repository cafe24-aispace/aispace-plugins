...
**활성화 규칙**: 이 스킬을 읽게 됐다면 — 슬래시 명령(`/aispace`)·수동 로드·자동 활성화 등
경위와 무관하게 — **바로 다음 응답에서 아래 온보딩 1단계를 시작한다.** 스킬 요약, 로드 상태
보고, "무엇을 도와드릴까요?" 같은 열린 질문으로 끝내지 않는다. "스킬 발동된 거야?" 같은
상태 질문에도 "네, 지금부터 시작합니다" 한 줄 뒤 곧바로 1단계를 실행한다.
(예외: 사용자가 스킬 문서 자체의 검토·수정을 요청한 경우)
